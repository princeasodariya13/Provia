...
export function ContactFooter({ contact, links }: Props) {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-[#06060A] py-36 md:py-48">
      {/* Background */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 sm:px-10 md:px-16 text-center">
        ...
        <motion.p
          ...
          className="text-[11px] font-black uppercase tracking-[0.3em] text-white/25 mb-6"
        >
          07 · Let's Connect
        </motion.p>
        ...
        {contact?.location && (
          <motion.div
            ...
            className="flex items-center justify-center gap-2 text-sm text-white/25 mb-10 font-medium"
          >
          ...
          >
            {links.map((link, i) => (
              <a
                ...
                className="inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full border border-white/10 bg-white/[0.04] text-sm text-white/55 hover:text-white hover:border-violet-500/40 hover:bg-violet-500/10 backdrop-blur-sm transition-all duration-300"
              >
