  ...
  return (
    ...
    >
      <div className="flex size-full items-center justify-center">
        <div className="w-[150vw] md:w-[120vw] shrink-0 scale-110">
          <div
            style={{
              transform: "rotateZ(-12deg) scale(1.1)",
            }}
            className="relative grid size-full origin-center grid-cols-4 md:grid-cols-6 gap-4"
          >
            {chunks.map((subarray, colIndex) => (
              <motion.div
                animate={{ y: colIndex % 2 === 0 ? "-20%" : "20%" }}
                transition={{
                  duration: colIndex % 2 === 0 ? 30 : 40,
                  repeat: Infinity,
                  repeatType: "reverse",
                  ease: "linear",
                }}
              ...
              >
                {/* Double the array for seamless illusion or just show more images */}
                {[...subarray, ...subarray, ...subarray].map((image, imageIndex) => (
                  <div className="relative w-full" key={imageIndex + image}>
                    <motion.img
                      ...
                      src={image}
                      alt={`Portfolio Preview ${imageIndex}`}
